...
app.post('/encryptVote', async (req, res) => {
  ...
  const encryptedVote = `zama_fhe_${electionId}_${candidateIndex}_${Date.now()}_${Math.random().toString(36).slice(2)}`
...
app.post('/tallyVotes', async (req, res) => {
  ...
  if (!Array.isArray(encryptedVotes)) return res.status(400).json({ error: 'invalid body' })
