...
func (h *Handlers) Login(c *fiber.Ctx) error {
    ...
    if err != nil { return c.Status(401).JSON(fiber.Map{"error": "invalid credentials"}) }
    tok, _ := h.jwt.Make(u.ID)
...
func (h *Handlers) SubmitVote(c *fiber.Ctx) error {
    var body struct{ ElectionID string; CandidateIndex int }
    if err := c.BodyParser(&body); err != nil { return c.Status(400).JSON(fiber.Map{"error": "invalid body"}) }
    uid := c.Locals("userID").(string)
    resp, err := h.voting.SubmitVote(uid, body.ElectionID, body.CandidateIndex)
    if err != nil { return c.Status(412).JSON(fiber.Map{"error": err.Error()}) }
