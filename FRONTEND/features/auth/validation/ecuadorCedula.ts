...
export function validateEcuadorCedula(
  cedulaRaw: string
  ...
  if (!/^\d{10}$/.test(cedula)) {
    return {
      valid: false,
      reason: "La cédula debe tener exactamente 10 dígitos numéricos.",
      type: "unknown",
    };
  }
  ...
  if (!(provincia >= 1 && provincia <= 24) && !allowedSpecial.has(provincia)) {
    return {
      valid: false,
      reason: "Código de provincia inválido (01–24, o casos especiales 30/50).",
      type: "unknown",
    };
  }
  ...
  if (tercer >= 0 && tercer <= 5) {
    ...
    return ok
      ...
      : {
          valid: false,
          reason: "Dígito verificador inválido (módulo 10).",
          type: "natural",
        };
  }
  ...
  if (tercer === 6) {
    ...
    return {
      valid: false,
      reason:
        "El tercer dígito 6 corresponde a entidad pública (RUC), no a cédula de 10 dígitos.",
      type: "public",
    };
  }
  ...
  if (tercer === 9) {
    ...
    return {
      valid: false,
      reason:
        "El tercer dígito 9 corresponde a sociedad privada (RUC), no a cédula de 10 dígitos.",
      type: "private",
    };
  }
  ...
  return {
    valid: false,
    reason: "Formato de cédula no reconocido.",
    type: "unknown",
  };
}
